...
def run_evidently_report(data_version="original"):
    ...
    drift_html = f"../reports/data_drift_report_{data_version}.html"
    perf_html = f"../reports/regression_performance_report_{data_version}.html"
    ...
    with mlflow.start_run(run_name=f"evidently_report_{data_version}", nested=True):
        mlflow.log_artifact(drift_html)
        mlflow.log_artifact(perf_html)

    print(f"Evidently reports generated and logged to MLflow for data_version={data_version}.")